...
GAME_NAME="rayshooter"

# Set your sources here (absolute paths!)
# Example with two source folders:
# SOURCES="src/*.c src/submodule/*.c"
SOURCES="${PWD}/src/*.cpp ${PWD}/src/context/*.cpp ${PWD}/src/entities/*.cpp ${PWD}/src/screens/*.cpp"

# Set your raylib/src location here (relative path!)
RAYLIB_SRC="libs/raylib/"
...
LINK_FLAGS="-static-libgcc -static-libstdc++ -flto -lkernel32 -luser32 -lshell32 -lwinmm -lgdi32 -lopengl32"
# Debug changes to flags
if [ -n "$BUILD_DEBUG" ]; then
    OUTPUT_DIR="builds-debug/windows-mingw"
    COMPILATION_FLAGS="-std=c99 -O0 -g"
    FINAL_COMPILE_FLAGS=""
    LINK_FLAGS="-static-libgcc -static-libstdc++ -lkernel32 -luser32 -lshell32 -lwinmm -lgdi32 -lopengl32"
fi
...
mkdir -p $OUTPUT_DIR
cp $PWD/assets $OUTPUT_DIR/assets -r
cd $OUTPUT_DIR
